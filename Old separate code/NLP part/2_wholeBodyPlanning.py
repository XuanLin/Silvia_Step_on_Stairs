...
#------------------------------------ Constraint 5 -----------------------------------------------------------------
# support polygon 1: [750, 1180], [1500, 780], [750, 380]
# 0.53333*CoMx+CoMy-1580<0
# 0.53333*CoMx-CoMy-20<0
# 750-CoMx<0
# support polygon 2: [1250, 1326.5], [1250, 526.5], [500, 924]
...
def supportHexagonConstraint(result, x, grad, step):
    ...
    if (step <= N/2):
       #Polygon 1
       result[:] = np.array([0.53333*CoMx+CoMy-1580, 0.53333*CoMx-CoMy-20, 750-CoMx])
       if grad.size > 0:
            grad[:] = np.zeros([3, 24*N])

            grad[0, step*24+3] = 0.53333
            grad[0, step*24+4] = 1.0

            grad[1, step*24+3] = 0.53333
       ...
       #Polygon 2
       result[:] = np.array([-0.53667*CoMx+CoMy-655.66667, -0.53*CoMx-CoMy+1189, CoMx-1250])
       if grad.size > 0:
            grad[:] = np.zeros([3, 24*N])

            grad[0, step*24+3] = -0.53667
            grad[0, step*24+4] = 1.0

            grad[1, step*24+3] = -0.53
            grad[1, step*24+4] = -1.0
